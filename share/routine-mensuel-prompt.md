...
# Maintenance MOS (avant de rédiger le bilan)

Lance via **`Bash`** :
```
jarvis-mos prune 90
```
Cela supprime les events MOS plus vieux que 90 jours (rétention raisonnable
pour audits cross-session). Reporte les counts `before/deleted/after` en
note de bas de bilan : `_MOS prune mensuel : X events purgés, Y conservés_`.
